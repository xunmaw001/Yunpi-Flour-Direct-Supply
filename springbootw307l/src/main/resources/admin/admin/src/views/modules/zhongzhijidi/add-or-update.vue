<template>
  <div class="addEdit-block">
    ...
    >
      <el-row>
      <el-col :span="12">
        <el-form-item class="input" v-if="type!='info'"  label="种植基地" prop="zhongzhijidi">
          <el-input v-model="ruleForm.zhongzhijidi" 
              placeholder="种植基地" clearable  :readonly="ro.zhongzhijidi"></el-input>
        </el-form-item>
        <div v-else>
      ...
      <el-col :span="24">  
        ...
        <div v-else>
          ...
          <el-row>
            <el-col :span="24">
              <el-form-item v-if="type!='info'"  label="基地介绍" prop="jidijieshao">
                <editor 
                    style="min-width: 200px; max-width: 600px;"
                    v-model="ruleForm.jidijieshao" 
                    class="editor" 
                    action="file/upload">
                </editor>
              </el-form-item>
              <div v-else>
                <el-form-item v-if="ruleForm.jidijieshao" label="基地介绍" prop="jidijieshao">
                    <span v-html="ruleForm.jidijieshao"></span>
                </el-form-item>
              </div>
            </el-col>
          </el-row>
...
export default {
  data() {
	...
	crazilynum : false,
      ...
      rules: {
          ...
          jidijieshao: [
                { required: true, message: '基地介绍不能为空', trigger: 'blur' },
          ],
          dizhi: [
  ...
  methods: {
	...
	this.ruleForm.jidijieshao = this.ruleForm.jidijieshao.replace(reg,'../../../springbootw307l/upload');
        } else {
          this.$message.error(data.msg);
        }
      });
    ...
    onSubmit() {
      ...
      this.$refs["ruleForm"].validate(valid => {
        if (valid) {
          this.$http({
            url: `zhongzhijidi/${!this.ruleForm.id ? "save" : "update"}`,
            method: "post",
            data: this.ruleForm
          }).then(({ data }) => {
            if (data && data.code === 0) {
              this.$message({
                ...
                onClose: () => {
                  ...
                  this.parent.zhongzhijidiCrossAddOrUpdateFlag = false;
                  this.parent.search();
