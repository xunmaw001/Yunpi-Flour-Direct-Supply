    ...
    >
      ...
      <el-col :span="12">
        <el-form-item class="select" v-if="type!='info'"  label="种植基地" prop="zhongzhijidi">
          <el-select v-model="ruleForm.zhongzhijidi" placeholder="请选择种植基地">
            <el-option
        ...
        <div v-else>
          <el-form-item class="input" label="种植基地" prop="zhongzhijidi">
      ...
      <el-col :span="24">  
        <el-form-item class="upload" v-if="type!='info' && !ro.caishoutupian" label="采收图片" prop="caishoutupian">
          <file-upload
          tip="点击上传采收图片"
          action="file/upload"
          :limit="3"
          :multiple="true"
          :fileUrls="ruleForm.caishoutupian?ruleForm.caishoutupian:''"
          @change="caishoutupianUploadChange"
          ></file-upload>
        </el-form-item>
        <div v-else>
          <el-form-item v-if="ruleForm.caishoutupian" label="采收图片" prop="caishoutupian">
            <img style="margin-right:20px;" v-bind:key="index" v-for="(item,index) in ruleForm.caishoutupian.split(',')" :src="item" width="100" height="100">
          </el-form-item>
        </div>
      </el-col>
      </el-row>
          <el-row>
            <el-col :span="24">
              <el-form-item v-if="type!='info'"  label="采收现场" prop="caishouxianchang">
                <editor 
                    style="min-width: 200px; max-width: 600px;"
                    v-model="ruleForm.caishouxianchang" 
                    class="editor" 
                    action="file/upload">
                </editor>
              </el-form-item>
              <div v-else>
                <el-form-item v-if="ruleForm.caishouxianchang" label="采收现场" prop="caishouxianchang">
                    <span v-html="ruleForm.caishouxianchang"></span>
                </el-form-item>
              </div>
            </el-col>
          </el-row>
...
export default {
  data() {
	...
	caishoutupian : false,
      ...
      rules: {
          zhongzhijidi: [
                { required: true, message: '种植基地不能为空', trigger: 'blur' },
          ],
          caishouxianchang: [
                { required: true, message: '采收现场不能为空', trigger: 'blur' },
          ],
          caishoutupian: [
                { required: true, message: '采收图片不能为空', trigger: 'blur' },
          ],
      }
    };
  },
  ...
  methods: {
    ...
    info(id) {
      this.$http({
        url: `yunpixiaomaicaishou/info/${id}`,
        method: "get"
      }).then(({ data }) => {
        if (data && data.code === 0) {
	...
	this.ruleForm.caishouxianchang = this.ruleForm.caishouxianchang.replace(reg,'../../../springbootw307l/upload');
        } else {
          this.$message.error(data.msg);
        }
      });
    ...
    onSubmit() {
      ...
      this.$refs["ruleForm"].validate(valid => {
        if (valid) {
          this.$http({
            url: `yunpixiaomaicaishou/${!this.ruleForm.id ? "save" : "update"}`,
            method: "post",
            data: this.ruleForm
          }).then(({ data }) => {
            if (data && data.code === 0) {
              this.$message({
                ...
                onClose: () => {
                  ...
                  this.parent.yunpixiaomaicaishouCrossAddOrUpdateFlag = false;
                  this.parent.search();
    ...
    caishoutupianUploadChange(fileUrls) {
	this.ruleForm.caishoutupian = fileUrls;
			this.addEditUploadStyleChange()
    },
	addEditStyleChange() {
	  this.$nextTick(()=>{
