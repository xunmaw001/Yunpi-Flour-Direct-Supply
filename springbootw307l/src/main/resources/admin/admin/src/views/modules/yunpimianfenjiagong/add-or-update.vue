    ...
    >
      ...
      <el-col :span="12">
        ...
        <div v-else>
          ...
          <el-row>
            <el-col :span="24">
              <el-form-item v-if="type!='info'"  label="面粉制作" prop="mianfenzhizuo">
                <editor 
                    style="min-width: 200px; max-width: 600px;"
                    v-model="ruleForm.mianfenzhizuo" 
                    class="editor" 
                    action="file/upload">
                </editor>
              </el-form-item>
              <div v-else>
                <el-form-item v-if="ruleForm.mianfenzhizuo" label="面粉制作" prop="mianfenzhizuo">
                    <span v-html="ruleForm.mianfenzhizuo"></span>
                </el-form-item>
              </div>
            </el-col>
          </el-row>
...
export default {
  data() {
	...
	beizhu : false,
      ...
      rules: {
          mianfenzhizuo: [
                { required: true, message: '面粉制作不能为空', trigger: 'blur' },
          ],
          zhongzhijidi: [
  ...
  methods: {
    ...
    info(id) {
      this.$http({
        url: `yunpimianfenjiagong/info/${id}`,
        method: "get"
      }).then(({ data }) => {
        if (data && data.code === 0) {
	...
	this.ruleForm.mianfenzhizuo = this.ruleForm.mianfenzhizuo.replace(reg,'../../../springbootw307l/upload');
        } else {
          this.$message.error(data.msg);
        }
      });
    ...
    onSubmit() {
      ...
      this.$refs["ruleForm"].validate(valid => {
        if (valid) {
          this.$http({
            url: `yunpimianfenjiagong/${!this.ruleForm.id ? "save" : "update"}`,
            method: "post",
            data: this.ruleForm
          }).then(({ data }) => {
            if (data && data.code === 0) {
              this.$message({
                ...
                onClose: () => {
                  ...
                  this.parent.yunpimianfenjiagongCrossAddOrUpdateFlag = false;
                  this.parent.search();
