	...
	<view class="content">
		<form class="app-update-pv">
				...
				<picker @change="zhongzhijidiChange" :value="zhongzhijidiIndex" :range="zhongzhijidiOptions">
					<view :style='{"boxShadow":"0 0 0px rgba(0,0,0,.6) inset","backgroundColor":"rgba(255, 255, 255, 0)","borderColor":"rgba(215, 215, 215, 1)","borderRadius":"0px","color":"rgba(255, 144, 0, 1)","textAlign":"left","borderWidth":"2rpx","fontSize":"28rpx","borderStyle":"solid","height":"88rpx"}' class="uni-input">{{zhongzhijidiOptions[zhongzhijidiIndex]}}</view>
				</picker>
			</view>
				...
				<view :style='{"width":"170rpx","fontSize":"28rpx","color":"rgba(147, 193, 7, 1)","textAlign":"left"}' class="title">备注</view>
				<input :style='{"boxShadow":"0 0 0px rgba(0,0,0,.6) inset","borderColor":"rgba(215, 215, 215, 1)","borderRadius":"0px","color":"rgba(255, 144, 0, 1)","textAlign":"left","borderWidth":"2rpx","fontSize":"28rpx","borderStyle":"solid","height":"88rpx"}' :disabled="ro.beizhu" v-model="ruleForm.beizhu" placeholder="备注"></input>
			</view>
			...
			<!-- 否 -->
				...
				<view :style='{"width":"170rpx","fontSize":"28rpx","color":"rgba(147, 193, 7, 1)","textAlign":"left"}' class="title">面粉制作</view>
				<textarea :style='{"boxShadow":"0 0 0px rgba(0,0,0,.6) inset","backgroundColor":"rgba(255, 255, 255, 0)","borderColor":"rgba(215, 215, 215, 1)","borderRadius":"0px","color":"rgba(255, 144, 0, 1)","textAlign":"left","borderWidth":"2rpx","fontSize":"28rpx","borderStyle":"solid","height":"280rpx"}' v-model="ruleForm.mianfenzhizuo" placeholder="面粉制作"></textarea>
			</view>
	...
	export default {
		...
		async onLoad(options) {
			...
			if (options.id) {
				...
				// 获取信息
				res = await this.$api.info(`yunpimianfenjiagong`, this.ruleForm.id);
			...
			if(options.cross){
				...
				for (var o in obj){
					if(o=='mianfenzhizuo'){
					this.ruleForm.mianfenzhizuo = obj[o];
					this.ro.mianfenzhizuo = true;
					continue;
					}
					if(o=='zhongzhijidi'){
		...
		methods: {
			styleChange() {
				this.$nextTick(()=>{
					...
					// })
																		...
																		// 下二随
			async zhongzhijidiChange (e) {
				this.zhongzhijidiIndex = e.target.value
				this.ruleForm.zhongzhijidi = this.zhongzhijidiOptions[this.zhongzhijidiIndex]
				let res = await this.$api.follow(`zhongzhijidi`, `zhongzhijidi`,{
					columnValue: this.ruleForm.zhongzhijidi
				});
																																							},
			...
			async onSubmitTap() {
																																																																																																																																																																												if((!this.ruleForm.mianfenzhizuo)){
					this.$utils.msg(`面粉制作不能为空`);
					return
				}
					...
					return
																																																																												...
																																																																												if(this.ruleForm.id){
					await this.$api.update(`yunpimianfenjiagong`, this.ruleForm);
				}else{
					await this.$api.add(`yunpimianfenjiagong`, this.ruleForm);
				}
