	...
	<mescroll-uni @init="mescrollInit" :up="upOption" :down="downOption" @down="downCallback" @up="upCallback" :bottom="100">
		<view class="container">
				...
				<view class="content" :style='{"color":"rgba(93, 87, 87, 1)","fontSize":"28rpx"}'>
					<rich-text style="font-size: 33upx;line-height: 50upx;letter-spacing: 5upx;" :nodes="detail.caishouxianchang"></rich-text>
				</view>
			</view>
...
<script>
	export default {
		...
		methods: {
			// 支付
			onPayTap(){
				uni.setStorageSync('paytable','yunpixiaomaicaishou');
			...
			onAcrossTap(tableName){
     			uni.setStorageSync('crossTable','yunpixiaomaicaishou');
				uni.setStorageSync(`crossObj`, this.detail);
				this.$utils.jump(`../${tableName}/add-or-update?cross=true`);
			},
			// 获取详情
			async init(){
				let res = await this.$api.info('yunpixiaomaicaishou', this.id);
				this.detail = res.data;
																				//修改富文本的图片样式
				this.detail.caishouxianchang = this.detail.caishouxianchang.replace(/img src/gi,"img style=\"width:100%;\" src");
																// 轮播图片
				this.swiperList = this.detail.caishoutupian ? this.detail.caishoutupian.split(",") : [];
											},
			...
			// 添加评论
			onCommentTap() {
				this.$utils.jump(`../discussyunpixiaomaicaishou/add-or-update?refid=${this.id}`)
			},
			// 点赞
			async zan() {
				let _this = this;
				uni.showModal({
					...
					success: async function(res) {
						if (res.confirm) {
							...
							await _this.$api.update('yunpixiaomaicaishou', _this.detail);
			...
			async onFinishTap() {
				...
				await this.$api.update('yunpixiaomaicaishou', this.detail);
				this.$utils.msg('审核成功');
